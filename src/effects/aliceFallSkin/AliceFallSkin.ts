...
import spiralPng from './spiral.png'
...
export function mountAliceFallSkin(
  ...
  // 底部旋涡（设计图，静止，不掉落）— 贴底按原始比例展示，顶部羽化融入背景
  const spiralWrap = document.createElement('div')
  spiralWrap.style.cssText =
    'position:absolute;left:0;bottom:0;width:100%;z-index:1;pointer-events:none;' +
  ...
  spiralImg.src = spiralPng
  spiralImg.style.cssText = 'display:block;width:100%;height:auto;'
  spiralWrap.appendChild(spiralImg)
  sheet.appendChild(spiralWrap)
  ...
  function tick(dt: number) {
    ...
    if (fallen) {
      ...
      if (!settled && (calmFrames > 24 || fallFrames > 180)) {
        for (const b of bodies) {
          // 保留落定瞬间的旋转方向（vrot 接近 0 时随机分一个），幅度 0.005–0.02 rad/帧
          const sign = b.vrot === 0 ? (Math.random() < 0.5 ? -1 : 1) : b.vrot > 0 ? 1 : -1
          b.vrot = sign * (0.005 + Math.random() * 0.015)
        }
        settled = true
      }
    }
